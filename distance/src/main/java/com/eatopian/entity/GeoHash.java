...
public final class GeoHash {
	...
	public static GeoHash fromGeohashString(String geohash) {
		...
		return GeoHash.withCharacterPrecision(points[0], points[1],
				geohash.length());
	}
	...
	public final Point getBoundingBoxCenterPoint() {
		double centerLatitude = (boundingBox[0].getLatitude() + boundingBox[1]
				.getLatitude()) / 2;
		double centerLongitude = (boundingBox[0].getLongitude() + boundingBox[1]
				.getLongitude()) / 2;
	...
	private static GeoHash recombineLatLonBitsToHash(long[] latBits,
			long[] lonBits) {
		long bits = 0;
		byte significantBits = 0;
